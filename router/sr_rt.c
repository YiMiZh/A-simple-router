...
/* Find an entry with longest prefix match on given destination address. */
struct sr_rt *sr_rt_lookup(struct sr_rt *head, uint32_t ip) {
    struct sr_rt *res_rt = NULL;
    uint32_t res_mask = 0;
    struct sr_rt *cur_rt = head;
    while (cur_rt) {
        uint32_t dest = cur_rt->dest.s_addr;
        uint32_t mask = cur_rt->mask.s_addr;
        uint32_t prefix = dest & mask;
        if (((prefix & ip) == prefix) && (mask > res_mask)) {
            res_rt = cur_rt;
            res_mask = mask;
        }
        cur_rt = cur_rt->next;
